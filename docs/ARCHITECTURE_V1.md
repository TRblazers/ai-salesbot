...
**Goal:** Natural language ? intent/parameters ? safe, parameterized SQL ? answer in Slack.

`mermaid
flowchart LR
  U[User in Slack] -->|/salesbot or DM| S[Slack App]
  S -->|HTTP POST| API[Azure Function API (/ask)]
  API -->|chat/completions| AOAI[Azure OpenAI (GPT-4o)]
  API -->|Parameterized SQL (RO)| SQL[(Azure SQL Database)]
  API -->|Structured logs| AI[Azure Application Insights]
  API -->|Secrets (keys/conn strings)| KV[Azure Key Vault]
...
Verify the file:
`powershell
notepad .\docs\ARCHITECTURE_V1.md

Set-Content -Path "docs\ARCHITECTURE_V1.md" -Value @"
# Architecture (v1)
...
Set-Content -Path "docs\PROJECT_CHARTER.md" -Value @"
